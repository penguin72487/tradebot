...
print("Using GPU for XGBoost:", xgb.__version__)
...
norm_cols = [col for col in all_features if col in df.columns]
...
def backtest_strategy(df, selected_features):
    ...
    for i in range(len(years) - 1):
        ...
        model = xgb.XGBRegressor(
            ...
            n_estimators=300,
            tree_method='hist',
            device='cuda'  # 告訴新版 XGBoost 要用 GPU
        )

        model.fit(X_train, y_train)  # 直接 DataFrame，就好！


        # test_df = test_df.copy()
        import cupy as cp
        X_test_gpu = cp.asarray(X_test.values)  # 把 CPU 上的 pandas.DataFrame 轉成 GPU 上的 cupy.ndarray
        test_df.loc[:, 'predicted_return'] = model.predict(X_test_gpu)
        test_df.loc[:, 'true_return'] = y_test.values
...
def plot_strategies(strategies):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'ga_feature_selection_results.png'))
    plt.show()

...
years.sort()
# 去掉最後一年
years = years[1:]  # 去掉前後一年，因為模擬的時候沒有前後續數據來計算報酬率
...
for n in top_n_list:
    plt.plot(years, years_top_cum_returns[n], marker='o', linestyle='-', label=f'top {n} cum returns', color=color_map[str(n)])
    plt.plot(years, years_bottom_cum_returns[n], marker='v', linestyle='--', label=f'bottom {n} cum returns', color=color_map[str(n)])
    plt.plot(years, years_top_bottom_cum_returns[n], marker='s', linestyle=':', label=f'top-bottom {n} cum returns', color=color_map[str(n)])
...
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(base_dir, 'upper_bound_Ex_cumulative_returns.png'))
plt.show()

...
def plot_annual_return_summary():
    # 讀取 annual return 資料
    base_dir = os.path.join("FinTechML", "mid", "TSMCMediaTekresult")
    ...
    output_annual = os.path.join("FinTechML", "mid", "TSMCMediaTekresult", "annual_return_plot.png")
...
def plot_combined_cumulative_return():
    ...
    # 設定 X 軸只顯示年份
    plt.gca().xaxis.set_major_locator(mdates.YearLocator())         # 每年顯示一個
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y')) # 格式是「年」
...
def main():
    ...
    # 第0張圖：累積報酬率圖 + SMA256
    df['TSMC_return'] = df['TSMC_price'].pct_change().fillna(0)
    df['MediaTek_return'] = df['MediaTek_price'].pct_change().fillna(0)

    df['TSMC_cum_return'] = (1 + df['TSMC_return']).cumprod()
    df['MediaTek_cum_return'] = (1 + df['MediaTek_return']).cumprod()

    # 累積報酬率的 256 日移動平均
    df['TSMC_cum_return_sma256'] = df['TSMC_cum_return'].rolling(window=256, min_periods=1).mean()
    df['MediaTek_cum_return_sma256'] = df['MediaTek_cum_return'].rolling(window=256, min_periods=1).mean()

    plt.figure(figsize=(14, 6))
    plt.plot(df['Date'], df['TSMC_cum_return'], label='TSMC Cumulative Return', color='blue', alpha=0.5)
    plt.plot(df['Date'], df['TSMC_cum_return_sma256'], label='TSMC SMA256', color='blue', linestyle='--')

    plt.plot(df['Date'], df['MediaTek_cum_return'], label='MediaTek Cumulative Return', color='green', alpha=0.5)
    plt.plot(df['Date'], df['MediaTek_cum_return_sma256'], label='MediaTek SMA256', color='green', linestyle='--')

    plt.title('Cumulative Return of TSMC and MediaTek (with 256-day SMA)')
    plt.xlabel('Date')
    ...
    # 第一張圖：各自的 Z-score
    plt.figure(figsize=(14, 6))
    plt.plot(df['Date'], df['TSMC_zscore'], label='TSMC Z-score', color='blue')
    plt.plot(df['Date'], df['MediaTek_zscore'], label='MediaTek Z-score', color='green')
    plt.axhline(0, color='gray', linestyle='--')
    plt.title('Z-score (256-day) of TSMC and MediaTek')
    plt.xlabel('Date')
    plt.ylabel('Z-score')
    plt.legend()
    plt.grid(True)
    ...
    # 第二張圖：Z-score 差
    plt.figure(figsize=(14, 6))
    plt.plot(df['Date'], df['Zscore_diff'], label='TSMC Z - MediaTek Z', color='purple')
    plt.axhline(0, color='black', linestyle='--')
    plt.axhline(1, color='green', linestyle='--', label='Z Diff = 1')
    plt.axhline(-1, color='red', linestyle='--', label='Z Diff = -1')
    plt.title('Z-score Difference (TSMC - MediaTek)')
    plt.xlabel('Date')

...
# GA_SCRIPT = "gaBTC.py"  # 遺傳演算法策略腳本
GA_SCRIPT = "gaBybit.py"
...
while True:
    ...
    # Ga 🧬 遺傳演算法執行時段
        ...
        if current_task != GA_SCRIPT:
            ...
            # 檢查 GA 程序是否意外結束（崩潰/退出），若是則重啟
            if current_process is None:
                log.info(f"⚠️ {datetime.datetime.now()} 發現 GA 程序不存在，重新啟動...")
                launch(GA_SCRIPT)
            else:
                ret = current_process.poll()
                if ret is not None:
                    log.warning(f"💥 {datetime.datetime.now()} GA 程序已停止（return code={ret}），重新啟動...")
                    kill_current_process()
                    launch(GA_SCRIPT)

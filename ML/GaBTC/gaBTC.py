...
if recompute:
    ...
    df.to_parquet(scaled_path, engine='pyarrow', index=False)
    logging.info(f"Wrote scaled results to: {scaled_path}")
...
# 4) 適存函式：累積報酬（用 log(1+pnl) 穩定）- L2 懲罰 - 交易成本
eval_fitness = make_fitness_fn(
    mode=fitness_mode,
    periods_per_year=periods_per_year,
    tcost=tcost,
...
for gen in range(generations):
    ...
    # mutation
    if mut_rate > 0:
        mut_mask = (torch.rand_like(children) < mut_rate)
        noise    = torch.randn_like(children) * mut_sigma
        children = children + mut_mask * noise
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(result_path, 'ga_vs_bh_equity.png'), dpi=150)
plt.close()

logging.info(f"Saved weights to: {weights_csv}")
logging.info(f"Saved equity curves to: {os.path.join(result_path, 'ga_vs_bh_equity.png')}")

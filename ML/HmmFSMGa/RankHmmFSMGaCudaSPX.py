...
def run_model():
    ...
    for n_states in range(46,1001):
        ...
        try:
            ...
            # 存圖主
            plot_strategy_curve(df, n_states, n_state_dir, sharpe, buy_and_hold_return, best_weights, save_name="strategy_curve.png")
...
def cross_val_model(df, n_states=5, result_dir=None):
    ...
    results.sort(key=lambda r: int(r["test_years"].split("–")[0]))
    ...
    plt.figure(figsize=(12, 6), dpi=300)  # 🎯 dpi 調高解析度
